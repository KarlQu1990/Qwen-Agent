...
class ReActChat(FnCallAgent):
    ...
    def _run(self, messages: List[Message], lang: Literal["en", "zh"] = "en", **kwargs) -> Iterator[List[Message]]:
        ...
        while num_llm_calls_available > 0:
            ...
            if action_input_json.get("files", []):
                files = [content.file for msg in messages for content in msg.content if content.file]
                action_input_json["files"] = files

            # Add the tool result
            observation = self._call_tool(action, action_input_json, messages=messages, **kwargs)
